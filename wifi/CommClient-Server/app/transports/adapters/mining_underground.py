"""
Mining / Underground transport adapter.

Family: ``mining_underground``

Provides detection of mining_underground-class network interfaces by matching
...
class MiningAdapter(BaseTransportAdapter):
    family = "mining_underground"
    display_name = "Mining / Underground"
    keywords = ['mining', 'shaft', 'tunnel-net', 'leakyfeeder', 'underground-mesh']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("mining_underground_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("mining_underground_serial_detect_failed", error=str(exc))

        logger.info("mining_underground_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("mining_underground_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("mining_underground_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("mining_underground_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("mining_underground_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("mining_underground_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("mining_underground_receive_failed", error=str(exc))
            return b""
