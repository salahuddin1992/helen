"""
Medical transport adapter.

Family: ``medical``

Provides detection of medical-class network interfaces by matching
...
class MedicalAdapter(BaseTransportAdapter):
    family = "medical"
    display_name = "Medical"
    keywords = ['hl7', 'dicom', 'medical', 'ekg', 'ecg', 'ventilator', 'infusion', 'icu-net']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("medical_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("medical_serial_detect_failed", error=str(exc))

        logger.info("medical_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("medical_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("medical_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("medical_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("medical_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("medical_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("medical_receive_failed", error=str(exc))
            return b""
