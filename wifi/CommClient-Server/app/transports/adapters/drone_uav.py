"""
Drone / UAV transport adapter.

Family: ``drone_uav``

Provides detection of drone_uav-class network interfaces by matching
...
class DroneUAVAdapter(BaseTransportAdapter):
    family = "drone_uav"
    display_name = "Drone / UAV"
    keywords = ['drone', 'uav', 'mavlink', 'telemetry', 'ardupilot', 'px4']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("drone_uav_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("drone_uav_serial_detect_failed", error=str(exc))

        logger.info("drone_uav_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("drone_uav_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("drone_uav_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("drone_uav_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("drone_uav_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("drone_uav_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("drone_uav_receive_failed", error=str(exc))
            return b""
