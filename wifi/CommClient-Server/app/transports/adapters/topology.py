"""
Topology transport adapter.

Family: ``topology``

Provides detection of topology-class network interfaces by matching
...
class TopologyAdapter(BaseTransportAdapter):
    family = "topology"
    display_name = "Topology"
    keywords = ['topology', 'graph', 'spine', 'leaf', 'tor']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("topology_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("topology_serial_detect_failed", error=str(exc))

        logger.info("topology_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("topology_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("topology_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("topology_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("topology_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("topology_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("topology_receive_failed", error=str(exc))
            return b""
