"""
Financial / Trading transport adapter.

Family: ``financial_trading``

Provides detection of financial_trading-class network interfaces by matching
...
class FinancialTradingAdapter(BaseTransportAdapter):
    family = "financial_trading"
    display_name = "Financial / Trading"
    keywords = ['fix', 'itch', 'ouch', 'trading', 'marketdata', 'exchange']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("financial_trading_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("financial_trading_serial_detect_failed", error=str(exc))

        logger.info("financial_trading_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("financial_trading_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("financial_trading_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("financial_trading_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("financial_trading_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("financial_trading_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("financial_trading_receive_failed", error=str(exc))
            return b""
