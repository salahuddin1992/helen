"""
Automotive transport adapter.

Family: ``automotive``

Provides detection of automotive-class network interfaces by matching
...
class AutomotiveAdapter(BaseTransportAdapter):
    family = "automotive"
    display_name = "Automotive"
    keywords = ['can', 'obd', 'automotive', 'candan', 'vehicle-bus', 'auto-ethernet']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("automotive_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("automotive_serial_detect_failed", error=str(exc))

        logger.info("automotive_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("automotive_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("automotive_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("automotive_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("automotive_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("automotive_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("automotive_receive_failed", error=str(exc))
            return b""
