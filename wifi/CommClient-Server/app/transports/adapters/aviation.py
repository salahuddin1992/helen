"""
Aviation transport adapter.

Family: ``aviation``

Provides detection of aviation-class network interfaces by matching
...
class AviationAdapter(BaseTransportAdapter):
    ...
    keywords = ['arinc', 'afdx', 'ads-b', 'acars', 'aviation', 'cpdlc']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("aviation_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("aviation_serial_detect_failed", error=str(exc))

        logger.info("aviation_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("aviation_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("aviation_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("aviation_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("aviation_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("aviation_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("aviation_receive_failed", error=str(exc))
            return b""
