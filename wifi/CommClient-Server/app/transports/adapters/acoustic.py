"""
Acoustic / Underwater transport adapter.

Family: ``acoustic``

Provides detection of acoustic-class network interfaces by matching
...
class AcousticAdapter(BaseTransportAdapter):
    family = "acoustic"
    display_name = "Acoustic / Underwater"
    keywords = ['acoustic', 'modem-acoustic', 'hydrophone', 'ultrasound-net']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("acoustic_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("acoustic_serial_detect_failed", error=str(exc))

        logger.info("acoustic_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("acoustic_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("acoustic_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("acoustic_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("acoustic_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("acoustic_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("acoustic_receive_failed", error=str(exc))
            return b""
