"""
Broadcast Media transport adapter.

Family: ``broadcast_media``

Provides detection of broadcast_media-class network interfaces by matching
...
class BroadcastMediaAdapter(BaseTransportAdapter):
    family = "broadcast_media"
    display_name = "Broadcast Media"
    keywords = ['smpte', 'ndi', 'st2110', 'aes67', 'broadcast', 'vsf-tr', 'audio-broadcast']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("broadcast_media_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("broadcast_media_serial_detect_failed", error=str(exc))

        logger.info("broadcast_media_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("broadcast_media_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("broadcast_media_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("broadcast_media_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("broadcast_media_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("broadcast_media_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("broadcast_media_receive_failed", error=str(exc))
            return b""
