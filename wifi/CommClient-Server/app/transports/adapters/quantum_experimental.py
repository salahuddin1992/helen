"""
Quantum (Experimental) transport adapter.

Family: ``quantum_experimental``

Provides detection of quantum_experimental-class network interfaces by matching
...
class QuantumAdapter(BaseTransportAdapter):
    family = "quantum_experimental"
    display_name = "Quantum (Experimental)"
    keywords = ['qkd', 'quantum', 'qnet', 'entangle']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("quantum_experimental_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("quantum_experimental_serial_detect_failed", error=str(exc))

        logger.info("quantum_experimental_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("quantum_experimental_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("quantum_experimental_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("quantum_experimental_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("quantum_experimental_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("quantum_experimental_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("quantum_experimental_receive_failed", error=str(exc))
            return b""
