"""
Railway transport adapter.

Family: ``railway``

Provides detection of railway-class network interfaces by matching
...
class RailwayAdapter(BaseTransportAdapter):
    family = "railway"
    display_name = "Railway"
    keywords = ['ertms', 'etcs', 'rail-', 'railnet', 'tcms', 'subway-net']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("railway_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("railway_serial_detect_failed", error=str(exc))

        logger.info("railway_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("railway_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("railway_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("railway_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("railway_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("railway_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("railway_receive_failed", error=str(exc))
            return b""
