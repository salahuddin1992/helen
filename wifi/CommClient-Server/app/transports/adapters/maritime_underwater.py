"""
Maritime / Underwater transport adapter.

Family: ``maritime_underwater``

Provides detection of maritime_underwater-class network interfaces by matching
...
class MaritimeAdapter(BaseTransportAdapter):
    family = "maritime_underwater"
    display_name = "Maritime / Underwater"
    keywords = ['maritime', 'subsea', 'underwater', 'modem-um', 'ais', 'dvl', 'sonar']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("maritime_underwater_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("maritime_underwater_serial_detect_failed", error=str(exc))

        logger.info("maritime_underwater_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("maritime_underwater_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("maritime_underwater_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("maritime_underwater_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("maritime_underwater_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("maritime_underwater_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("maritime_underwater_receive_failed", error=str(exc))
            return b""
