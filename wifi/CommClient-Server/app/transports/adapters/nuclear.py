"""
Nuclear transport adapter.

Family: ``nuclear``

Provides detection of nuclear-class network interfaces by matching
...
class NuclearAdapter(BaseTransportAdapter):
    family = "nuclear"
    display_name = "Nuclear"
    keywords = ['nuclear-net', 'rad', 'reactor', 'iaea', 'safety-bus']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("nuclear_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("nuclear_serial_detect_failed", error=str(exc))

        logger.info("nuclear_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("nuclear_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("nuclear_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("nuclear_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("nuclear_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("nuclear_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("nuclear_receive_failed", error=str(exc))
            return b""
