"""
Military / Defense transport adapter.

Family: ``military_defense``

Provides detection of military_defense-class network interfaces by matching
...
class MilitaryAdapter(BaseTransportAdapter):
    family = "military_defense"
    display_name = "Military / Defense"
    keywords = ['mil-', 'milstd', 'havequick', 'link16', 'link22', 'saatcom', 'sincgars', 'tactical-ip']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("military_defense_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("military_defense_serial_detect_failed", error=str(exc))

        logger.info("military_defense_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("military_defense_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("military_defense_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("military_defense_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("military_defense_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("military_defense_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("military_defense_receive_failed", error=str(exc))
            return b""
