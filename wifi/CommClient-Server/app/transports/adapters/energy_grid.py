"""
Energy Grid transport adapter.

Family: ``energy_grid``

Provides detection of energy_grid-class network interfaces by matching
...
class EnergyGridAdapter(BaseTransportAdapter):
    family = "energy_grid"
    display_name = "Energy Grid"
    keywords = ['dnp3', 'iec61850', 'iec104', 'modbus-grid', 'smartgrid', 'scada-grid', 'amr']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("energy_grid_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("energy_grid_serial_detect_failed", error=str(exc))

        logger.info("energy_grid_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("energy_grid_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("energy_grid_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("energy_grid_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("energy_grid_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("energy_grid_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("energy_grid_receive_failed", error=str(exc))
            return b""
