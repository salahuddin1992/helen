"""
Deep Space transport adapter.

Family: ``deep_space``

Provides detection of deep_space-class network interfaces by matching
...
class DeepSpaceAdapter(BaseTransportAdapter):
    family = "deep_space"
    display_name = "Deep Space"
    keywords = ['dsn', 'deep-space', 'dtn', 'bp-bundle', 'ccsds', 'dsp']
    ...
    async def detect(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("deep_space_psutil_detect_failed", error=str(exc))
        ...
        try:
            ...
            logger.debug("deep_space_serial_detect_failed", error=str(exc))

        logger.info("deep_space_detection_complete", count=len(detected))
        return detected
    ...
    async def connect(self, interface: str, config: dict[str, Any]) -> Any:
        ...
        if interface.upper().startswith("COM") or interface.startswith("/dev/tty"):
            try:
                ...
                logger.info("deep_space_serial_connected", interface=interface)
                return conn
            except Exception as exc:
                logger.error("deep_space_serial_connect_failed", error=str(exc))
                raise
        ...
        logger.info("deep_space_tcp_connected", host=host, port=port)
    ...
    async def disconnect(self, connection_id: Any) -> bool:
        try:
            ...
            logger.error("deep_space_disconnect_failed", error=str(exc))
            return False
    ...
    async def send(self, connection_id: Any, data: bytes) -> int:
        try:
            ...
            logger.error("deep_space_send_failed", error=str(exc))
            return 0
    ...
    async def receive(
        ...
        try:
            ...
            logger.error("deep_space_receive_failed", error=str(exc))
            return b""
