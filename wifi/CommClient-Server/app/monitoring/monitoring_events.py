"""Monitoring-event bus — decoupled emit/subscribe.

Used by the alert manager to fire ``alert.fired`` events that the
audit chain or external dashboards can pick up.
"""
    ...
    def emit(self, name: str, payload: dict | None = None) -> int:
        ...
        for h in handlers + wildcard:
            try:
                ...
                logger.warning("mon_event_handler_failed",
...
def emit(event: str, payload: dict | None = None) -> int:
    return _Bus.instance().emit(event, payload)


def subscribe(event: str, handler: HandlerFn) -> None:
    _Bus.instance().subscribe(event, handler)
