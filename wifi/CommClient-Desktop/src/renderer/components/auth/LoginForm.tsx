...
const _log = AppLogger.create('LoginForm');
...
export const LoginForm: React.FC = () => {
  ...
  const { login, isLoading, error, setServerUrl: setAuthServerUrl, clearError } = useAuthStore();
  const {
    servers,
    phase,
    bestServer,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Catch obvious empty-field cases before hitting the API. Without
    // these guards, an autocomplete-clobbered field or a stray Enter
    // submits empty strings, the server returns a generic 400, and the
    // user sees "Login failed" with no hint about which field is wrong.
    const u = username.trim();
    const p = password.trim();
    if (!u || !p) {
      _log.warn('Login submit blocked: missing fields');
      return;
    }
        ...
        <div className="bg-surface-900 rounded-lg border border-surface-800 shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              ...
              {isLoading ? t('auth.logging_in') : t('auth.connect')}
            </button>
          </form>
